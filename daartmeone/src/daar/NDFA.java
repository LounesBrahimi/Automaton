...
public class NDFA {
	...
	public void arbreToNDFA(arbreSyntaxique arbre){
		int initiale = 0;
		int finale = 1;
		int nvEtat = finale + 1;
		numeroEtat = nvEtat;
		...
		if ((arbre.sousArbre.size() == 2) && ((arbre.racine == RegEx.CONCAT)
			...
			if (arbre.sousArbre.get(1).sousArbre.size() == 0) {
				...
				//-------- alternation -------
	//		if ((arbre.sousArbre.get(1).sousArbre.size() > 1) && (arbre.sousArbre.get(1).racine == RegEx.ALTERN)) {
						arbreToNDFARec(arbre.sousArbre.get(1), nvEtat, initiale, finale);
			//	} else {
					//-------- traiter les autres cas droite!!!!!-------
			//	}
	...
	public void arbreToNDFARec(arbreSyntaxique arbre, int etatRacine ,int initiale, int finale) {
